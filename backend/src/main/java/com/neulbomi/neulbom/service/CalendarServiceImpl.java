...
public class CalendarServiceImpl implements CalendarService {
	...
	public HashMap<String, JSONObject> getCalendarMonthRecord(int userSeq, String date) {
		...
		for (BloodSugar bloodSugar : list) {
			...
			if (record.get("bloodSugar") == null)
				bloodSugarList = new JSONArray();

			JSONObject obj = new JSONObject();
			obj.put("bsSeq", bloodSugar.getBsSeq());
			obj.put("bsCode", timeCodeToKR(bloodSugar.getBsCode()));
		...
		// 2. 혈압 기록 불러오기
		List<BloodPressure> bloodPressures = bloodPressureRepository.findByDelYnAndUserSeqAndBpDateStartsWith("n",
				userSeq, date);
		for (BloodPressure bloodPressure : bloodPressures) {
			...
			if (record.get("bloodPressure") == null)
				bloodPressureList = new JSONArray();

			JSONObject obj = new JSONObject();
			obj.put("bpSeq", bloodPressure.getBpSeq());
			obj.put("bpCode", timeCodeToKR(bloodPressure.getBpCode()));
		...
		for (Other other : alcohols) {
			...
			if (record.get(other.getCode()) == null)
				otherList = new JSONArray();
	...
	static String timeCodeToKR(String time) {
		String KR = "";
